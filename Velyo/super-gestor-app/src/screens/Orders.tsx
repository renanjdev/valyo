...
import { Plus, Filter, FileText, CheckCircle2, Clock } from 'lucide-react-native';

export default function Orders() {
  const orders = [
    { id: '1', client: 'João da Silva', date: '17/04/2026', status: 'Concluído', value: 'R$ 450,00' },
    { id: '2', client: 'Maria Oliveira', date: '17/04/2026', status: 'Pendente', value: 'R$ 800,00' },
  ];
    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      <View className="p-6">
        ...
        <View className="flex-row gap-2 mb-6">
          ...
          <TouchableOpacity className="bg-white px-4 py-2 rounded-full border border-slate-200">
            <Text className="text-slate-600 font-medium text-xs">Concluídas</Text>
          </TouchableOpacity>
        </View>
          ...
          renderItem={({ item }) => (
            <TouchableOpacity className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-4">
              <View className="flex-row justify-between items-start mb-2">
                <Text className="text-slate-900 font-bold text-lg">OS #2024-00{item.id}</Text>
              ...
              <Text className="text-slate-500 mb-4">{item.client}</Text>
